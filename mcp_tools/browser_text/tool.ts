/**
 * Browser Text tool -- extracts text content from an open browser page.
 */
 ...
 *
 * @param params - Must contain page_id. May contain selector.
 * @returns Object with extracted text content
 * @throws Error if page_id is missing or selector matches no elements
 */
async function handleRequest(params: RequestParams): Promise<{ text: string }> {
  ...
  try {
    ...
    if (selector) {
      ...
      text = body ? (await body.innerText()) : '';
    }
