// Package main implements the git_pull MCP tool.
package main
...
// Run pulls changes from a remote repository.
func Run(params Params) (string, error) {
	...
	cmd := exec.Command("git", "-C", path, "pull", remote, targetBranch)
	out, err := cmd.CombinedOutput()
	if err != nil {
		return "", fmt.Errorf("Failed to pull: %s", strings.TrimSpace(string(out)))
	}

	return fmt.Sprintf("Pulled '%s' from '%s' successfully", targetBranch, remote), nil
}
